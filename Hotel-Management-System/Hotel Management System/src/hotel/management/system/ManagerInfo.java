...
public class ManagerInfo extends JFrame {
	...
	private JPanel createButtonPanel() {
		...
		loadDataButton.addActionListener(e -> loadManagerData());
	...
	private void loadManagerData() {
		try (Connection conn = DatabaseConnection.getConnection();
			 PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM Employee WHERE job = ?")) {
			pstmt.setString(1, "Manager");
			try (ResultSet rs = pstmt.executeQuery()) {
				table.setModel(buildTableModel(rs));
			}
		} catch (SQLException e) {
			JOptionPane.showMessageDialog(this, "Error loading manager data: " + e.getMessage(),
